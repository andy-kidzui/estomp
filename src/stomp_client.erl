...
-export([connect/1, disconnect/1, subscribe/2, subscribe/3, ack/2, ack/3, send/3, unsubscribe/2]).
...
%% @spec connect(FsmRef) -> Reply
%% @doc Stomp API - send a connect message
connect(FsmRef) ->
    gen_fsm:send_event(FsmRef, {connect}).
